...
t = pd.Series(0, index=range(data.shape[0]))
...
print(t)
l2 = t.size
data_speed = pd.DataFrame(0, index=range(data.shape[0]), columns=range(2))
for l2 in range(l2,7,-1):
    data_speed.iloc[x2-6,0] = t.iloc[x2]
    data_speed.iloc[x2 - 6, 1] = 1/(t.iloc[x2]-t.iloc[x2-6])
#data_zero[0:] = data[0:]
#data.loc[1,1] = data
